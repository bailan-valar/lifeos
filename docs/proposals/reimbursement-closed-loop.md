...
| 文件路径 | 用途 | 状态 |
|---------|------|------|
| `types/reimbursement.ts` | 报销单类型定义 | ✅ 已实现 |
| `composables/useReimburse.ts` | 报销单 CRUD 和聚合查询 | ✅ 已实现 |
| `app-modules/billing/components/ReimburseDashboard.vue` | 报销仪表盘页面 | ✅ 已实现 |
| `app-modules/billing/components/ReimburseGroupDialog.vue` | 创建/编辑报销单对话框 | ✅ 已实现 |
| `app-modules/billing/components/ReimburseIncomeDialog.vue` | 记录报销回款对话框 | ✅ 已实现 |
| `app-modules/billing/components/ReimburseStatusBadge.vue` | 报销状态徽章组件 | ❌ 未实现（已内联到 BillListItem） |
| `app-modules/billing/components/ReimburseSummaryCard.vue` | 报销汇总统计卡片 | ❌ 未实现（已内联到 ReimburseDashboard） |
| `app-modules/billing/components/ReimburseGroupItem.vue` | 报销单列表项组件 | ❌ 未实现（已内联到 ReimburseDashboard） |

### 12.2 修改文件清单（方案B）

| 文件路径 | 修改内容 | 状态 |
|---------|---------|------|
| `types/bill.ts` | 新增 reimbursementId, reimbursementRole 字段 | ✅ 已实现 |
| `composables/useBills.ts` | 新增按报销ID查询、创建回款账单方法 | ✅ 已实现 |
| `stores/billing.ts` | TabId 类型扩展，新增 'reimburse' | ✅ 已实现 |
| `app-modules/billing/BillingView.vue` | 新增报销面板路由和组件渲染 | ✅ 已实现 |
| `app-modules/billing/components/layout/BillingSidebar.vue` | 新增"报销"导航项 | ✅ 已实现 |
| `app-modules/billing/components/layout/BillingMobileTabbar.vue` | 移动端报销入口 | ✅ 已实现 |
| `app-modules/billing/components/panels/BillsTabPanel.vue` | 新增报销筛选、统计、对话框状态和事件处理 | ✅ 已实现 |
| `app-modules/billing/components/BillContextMenu.vue` | 新增"加入报销单"/"从报销单移除"菜单项 | ✅ 已实现 |
| `app-modules/billing/components/BillListItem.vue` | 新增报销状态 badge（内联实现） | ✅ 已实现 |
| `app-modules/billing/components/BillList.vue` | 报销状态筛选；批量模式新增"创建报销单" | ⚠️ 部分实现（筛选未实现） |
| `app-modules/billing/components/BillDialog.vue` | 编辑模式展示报销关联信息 | ✅ 已实现 |
...
`Bill` 类型上已有 `isReimbursable?: boolean` 字段（`types/bill.ts:114`），导入流程的报销标记支持**部分已实现**：

| 链路节点 | 状态 | 说明 |
|---------|------|------|
| `ImportPreviewRow` 类型 | ✅ 已有 `isReimbursable?: boolean` | `types/bill.ts:377` |
| `ImportRecordItem` 类型 | ✅ 已有 `isReimbursable?: boolean` | `types/bill.ts:467` |
| `previewRowToRecordItem()` | ✅ 传递 `isReimbursable` | `types/bill.ts:408` |
| `ImportPreviewRow.vue` | ✅ 已有"可报销" checkbox | 桌面端预览行 |
| `createBillsBatch()` | ✅ 传递 `isReimbursable` | `composables/useBills.ts:474` |
| `MobileImportItemEditor.vue` | ❌ 缺少"可报销"开关 | 移动端编辑弹窗 |
| `ImportRule` 类型 | ❌ 无 `isReimbursable` 字段 | 规则不支持自动标记报销 |
| `ImportRuleFormData` 类型 | ❌ 无 `isReimbursable` 字段 | 规则表单无报销选项 |
| `ImportRuleForm.vue` | ❌ 无"可报销"开关 | 规则编辑表单 |
| `useImportRules.ts` | ❌ `applyRules()` 未传递 | 规则匹配不处理报销 |
| `services/csvImport.ts` | ❌ `buildImportRecordItem()` 未设置 | 默认值缺失 |
...
| 文件 | 修改内容 | 类型 | 状态 |
|------|---------|------|------|
| `types/bill.ts` | `ImportRecordItem` 接口新增 `isReimbursable?: boolean` | 修改 | ✅ 已完成 |
| `services/csvImport.ts` | `buildImportRecordItem()` 默认设置 `isReimbursable`（可通过规则覆盖） | 修改 | ❌ 待实现 |
| `app-modules/billing/components/ImportPreviewRow.vue` | 新增"可报销" checkbox（参照 `isSavable` 实现） | 修改 | ✅ 已完成 |
| `app-modules/billing/components/MobileImportItemEditor.vue` | 新增"可报销"开关（移动端） | 修改 | ❌ 待实现 |
| `composables/useBills.ts` | `createBillsBatch()` 构建 Bill 时传递 `isReimbursable` 字段 | 修改 | ✅ 已完成 |
| `types/bill.ts` | `ImportRule` / `ImportRuleFormData` 新增 `isReimbursable?: boolean` | 修改 | ❌ 待实现 |
| `composables/useImportRules.ts` | `applyRules()` / `buildRuleUpdates()` 传递 `isReimbursable` | 修改 | ❌ 待实现 |
| `app-modules/billing/components/ImportRuleForm.vue` | 规则表单新增"可报销"开关 | 修改 | ❌ 待实现 |
...
| 组件 | 排除父账单 | 扣除退款 | 排除报销 | 过滤 status | 备注 |
|------|----------|---------|---------|------------|------|
| BudgetDashboard（分类预算） | ✅ | ✅ | ✅ | ❌ | 已排除 `isReimbursable=true` |
| ProjectBudgetDashboard（项目预算） | ✅ | ✅ | ✅ | ❌ | 已排除 `isReimbursable=true` |
| CategoryDetailView（分类详情） | ❌ | ❌ | ❌ | ✅ | ⚠️ 已知不一致未修复 |

> ⚠️ CategoryDetailView 存在已知不一致：未排除 `hasChildren` 父账单（金额重复计算），未扣除退款，未排除报销。

> ⚠️ 当前预算排除逻辑使用 `isReimbursable` 标记判断，而非方案14.3中定义的按 `reimbursementId` + 报销单 `paid` 状态排除。这意味着只要标记为"可报销"就不计入预算，即使还未创建报销单或报销单未打款。后续可升级为按报销单 paid 状态精确排除。
...
| 文件 | 修改内容 | 说明 | 状态 |
|------|---------|------|------|
| `BudgetDashboard.vue` | `getDirectActual()` 增加报销排除 | 已排除 `isReimbursable=true` | ✅ 已实现（简化版） |
| `ProjectBudgetDashboard.vue` | `getDirectActual()` 增加报销排除 | 已排除 `isReimbursable=true` | ✅ 已实现（简化版） |
| `CategoryDetailView.vue` | `totalAmount` 和 `budgetUsage` 增加：排除 hasChildren + 扣除退款 + 扣除报销 | 修复现有不一致 + 新增报销排除 | ❌ 待实现 |
| **新增** `utils/budgetCalc.ts` | 提取共享的预算统计函数 `calcBudgetActual()` | 消除三个组件的重复逻辑 | ❌ 待实现 |
...
### 15.1 Phase 1 核心功能（方案B）

| 功能模块 | 状态 | 说明 |
|---------|------|------|
| Bill 新增 `reimbursementId` / `reimbursementRole` 字段 | ✅ 完成 | `types/bill.ts` |
| Bill 新增 `isReimbursable` 字段 | ✅ 完成 | `types/bill.ts:114` |
| 新增 `ReimbursementGroup` 类型 | ✅ 完成 | `types/reimbursement.ts` |
| 新增 `useReimburse` composable | ✅ 完成 | 报销单 CRUD、状态流转、回款记录 |
| 报销仪表盘页面 | ✅ 完成 | `ReimburseDashboard.vue` |
| 创建/编辑报销单对话框 | ✅ 完成 | `ReimburseGroupDialog.vue` |
| 记录报销回款对话框 | ✅ 完成 | `ReimburseIncomeDialog.vue` |
| 右键菜单报销操作 | ✅ 完成 | "加入报销单"/"从报销单移除"/"标记报销"/"取消报销" |
| 账单列表报销 badge | ✅ 完成 | "报销"/"待报销"/"报销回款" 三种 badge |
| 侧边栏报销 Tab 入口 | ✅ 完成 | 桌面端 + 移动端 |
| stores/billing.ts 报销 Tab | ✅ 完成 | `BillingTabId` 含 'reimburse' |
| BillingView 报销面板渲染 | ✅ 完成 | `?tab=reimburse` 路由支持 |
| 账单创建/批量创建传递 `isReimbursable` | ✅ 完成 | `useBills.ts` |
| 组件导入修复（BillsTabPanel / ReimburseDashboard） | ✅ 完成 | 添加显式 import 语句 |

### 15.2 Phase 1+ 导入场景增强

| 功能点 | 状态 | 说明 |
|--------|------|------|
| `ImportPreviewRow` 类型支持 `isReimbursable` | ✅ 完成 | `types/bill.ts:377` |
| `ImportRecordItem` 类型支持 `isReimbursable` | ✅ 完成 | `types/bill.ts:467` |
| `previewRowToRecordItem()` 传递 `isReimbursable` | ✅ 完成 | `types/bill.ts:408` |
| `ImportPreviewRow.vue` 桌面端"可报销" checkbox | ✅ 完成 | 桌面端导入预览行 |
| `createBillsBatch()` 传递 `isReimbursable` | ✅ 完成 | `useBills.ts:474` |
| `MobileImportItemEditor.vue` 移动端"可报销"开关 | ❌ 待实现 | 需参照 `isSavable` 实现 |
| `ImportRule` 类型新增 `isReimbursable` | ❌ 待实现 | 规则自动标记 |
| `ImportRuleFormData` 类型新增 `isReimbursable` | ❌ 待实现 | 规则表单数据 |
| `ImportRuleForm.vue` 规则表单"可报销"开关 | ❌ 待实现 | 规则编辑弹窗 |
| `useImportRules.ts` 传递 `isReimbursable` | ❌ 待实现 | `applyRules()` / `buildRuleUpdates()` |
| `services/csvImport.ts` 默认设置 `isReimbursable` | ❌ 待实现 | `buildImportRecordItem()` |

### 15.3 Phase 1+ 预算排除规则

| 功能点 | 状态 | 说明 |
|--------|------|------|
| BudgetDashboard 排除 `isReimbursable` 支出 | ✅ 完成 | `getDirectActual()` 中 `if (b.isReimbursable) return false` |
| ProjectBudgetDashboard 排除 `isReimbursable` 支出 | ✅ 完成 | 同上逻辑 |
| CategoryDetailView 排除报销支出 | ❌ 待实现 | 当前未排除 hasChildren/退款/报销 |
| 提取共享 `utils/budgetCalc.ts` | ❌ 待实现 | 消除三组件重复逻辑 |
| 升级为按报销单 paid 状态精确排除 | ❌ 待实现 | 当前仅按 `isReimbursable` 标记排除 |

### 15.4 已知问题

| 编号 | 问题 | 严重度 | 状态 |
|------|------|--------|------|
| BUG-1 | `ReimburseGroupDialog` / `ReimburseIncomeDialog` 未导入导致 Vue 组件解析警告 | 中 | ✅ 已修复 |
| TODO-1 | 预算排除逻辑仅按 `isReimbursable` 标记判断，未按报销单 paid 状态精确排除 | 低 | ❌ 待实现 |
| TODO-2 | `CategoryDetailView` 预算统计未排除 hasChildren/退款/报销 | 中 | ❌ 待实现 |
| TODO-3 | 导入规则不支持自动标记 `isReimbursable` | 低 | ❌ 待实现 |
| TODO-4 | 移动端导入编辑器缺少"可报销"开关 | 低 | ❌ 待实现 |
| TODO-5 | `BillList.vue` 报销状态筛选按钮未实现 | 低 | ❌ 待实现 |

### 15.5 设计决策记录

1. **报销单存储方案**：采用方案B-1（纯字段方案），报销单元数据存储在 `module_data`（key: `reimbursement/{id}`），运行时通过 `reimbursementId` 聚合关联账单。不新增独立集合。
2. **报销 badge 内联实现**：`ReimburseStatusBadge`、`ReimburseSummaryCard`、`ReimburseGroupItem` 三个组件方案中的独立组件实际内联到了 `BillListItem.vue` 和 `ReimburseDashboard.vue` 中，减少了组件数量但增加了单文件复杂度。如后续需要复用可提取为独立组件。
3. **预算排除简化实现**：预算统计中排除 `isReimbursable=true` 的支出，而非方案14.3中定义的按 `reimbursementId` + 报销单 paid 状态精确排除。这简化了实现（无需查询报销单状态），但语义略有差异：标记为"可报销"的支出立即不计入预算，即使还没创建报销单。
